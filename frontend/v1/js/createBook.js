...
document.getElementById('add-book-modal').addEventListener('show.bs.modal', () => {
    ...
    const authorDropdownElement = authorSelectDisplay;
    new bootstrap.Dropdown(authorDropdownElement);
    const shelfDropdownElement = shelfSelectDisplay;
    new bootstrap.Dropdown(shelfDropdownElement);
    const statusDropdownElement = statusSelectDisplay;
...
document.getElementById('saveBookBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        await fetchWithAuth(`${API_BASE}/book`, {
            method: 'POST',
            body: formData
        });
