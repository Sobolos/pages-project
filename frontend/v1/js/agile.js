...
function loadBoard(statuses, authors, books) {
    try {
        ...
        statuses.forEach((status, index) => {
            ...
            const booksInStatus = books.filter(book => book.status === status.id);
...
document.getElementById('saveStatusBtn').addEventListener('click', async () => {
    ...
    const request = JSON.stringify({ name: name, color: color })

    try {
        await fetchWithAuth(`${API_BASE}/status`, {headers: { 'Content-Type': 'application/json' },method: 'POST', body: request});
