...
async function loadBoard() {
    try {
        const [statusData, booksData, authorsData] = await Promise.all([
            fetchWithAuth(`${API_BASE}/statuses`),
            fetchWithAuth(`${API_BASE}/books`),
            fetchWithAuth(`${API_BASE}/authors`)
        ]);
        statuses = statusData.sort((a, b) => a.position - b.position); // Sort statuses by position
        authors = authorsData;
        ...
        statuses.forEach((status, index) => {
            ...
            title.className = 'status-title mb-0 flex-grow-1 text-center';
            ...
            editButton.className = 'btn btn-outline-warning btn-sm edit-status-btn';
            editButton.innerHTML = '✏️';
            editButton.addEventListener('click', () => openEditStatusModal(status));
            buttonGroup.appendChild(editButton);

            const deleteButton = document.createElement('button');
            deleteButton.className = 'btn btn-outline-danger btn-sm delete-status-btn';
            ...
            const booksInStatus = booksData.filter(book => book.status === status.id);
        ...
        document.getElementById('bookStatus').innerHTML = statuses.map(s => `<option value="${s.id}">${s.name}</option>`).join('');
    } catch (error) {
        console.error(error);
...
async function handleColumnDrop(e) {
    ...
    try {
        ...
        await fetchWithAuth(`${API_BASE}/statuses/reorder`, {
            method: 'PATCH',
            headers: {'Content-Type': 'application/json'},
...
function openEditStatusModal(status) {
    ...
    newSaveButton.addEventListener('click', async () => {
        ...
        try {
            await fetchWithAuth(`${API_BASE}/status/${status.id}`, {
                method: 'PUT',
                headers: {'Content-Type': 'application/json'},
...
async function deleteStatus(statusId) {
    ...
    try {
        await fetchWithAuth(`${API_BASE}/status/${statusId}`, {
            method: 'DELETE'
        });
        loadBoard();
    } catch (error) {
